...
3. **`/api/checkout` creates orders; payment is not wired yet.** It re-prices from the DB, validates inventory + coupon, and creates a PENDING `Order` (SSC-12). Pure money math lives in `src/lib/pricing.ts` (unit-tested). Taking payment (Stripe session + PAID webhook, inventory decrement) is SSC-13.
4. **Coupons/inventory are enforced at checkout, not in the cart UI.** Checkout validates coupon validity + inventory; there's no coupon field in the cart yet, and stock is decremented at payment (SSC-13), not at order creation.

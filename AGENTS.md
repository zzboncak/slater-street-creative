...
3. **`/api/checkout` re-prices, creates a PENDING order, and hands off to Stripe Checkout** (SSC-12 + SSC-13). Money math is in `src/lib/pricing.ts` (unit-tested); the Stripe client is `src/lib/stripe.ts` (needs `STRIPE_SECRET_KEY`, else 503). **No webhook yet** — the order stays `PENDING` after payment; a `checkout.session.completed` handler to mark it `PAID` and decrement inventory is the next payments ticket.
4. **Coupons/inventory are enforced at checkout, not in the cart UI.** Checkout validates coupon validity + inventory; there's no coupon field in the cart yet, and stock is decremented at payment confirmation (webhook, pending), not at order creation.

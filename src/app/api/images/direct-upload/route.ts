...
export async function POST() {
  ...
  try {
    ...
    if (!res.ok || !data?.success) {
      return NextResponse.json({ error: data?.errors || "Failed to create direct upload URL" }, { status: 500 });
    }
    return NextResponse.json({ uploadURL: data.result?.uploadURL });
  } catch {
    return NextResponse.json({ error: "Cloudflare API error" }, { status: 500 });
  }
}

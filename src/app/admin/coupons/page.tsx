    ...
    <div className="space-y-6">
      <form action={createCoupon} className="border rounded p-4 space-y-2">
        ...
        <div className="grid gap-2 sm:grid-cols-2">
          ...
          <input name="percentOff" placeholder="Percent off (0-100)" type="number" className="border rounded px-2 py-1" />
          <input name="amountOff" placeholder="Amount off (cents)" type="number" className="border rounded px-2 py-1" />
          <input name="validFrom" placeholder="Valid from (YYYY-MM-DD)" className="border rounded px-2 py-1" />
          <input name="validTo" placeholder="Valid to (YYYY-MM-DD)" className="border rounded px-2 py-1" />
          <input name="description" placeholder="Description" className="border rounded px-2 py-1 sm:col-span-2" />
        </div>
        ...
        <tbody>
          {coupons.map((c: Coupon) => (
            <tr key={c.id} className="border-b">
              ...
              <td>{[c.validFrom?.toISOString().slice(0,10), c.validTo?.toISOString().slice(0,10)].filter(Boolean).join(" → ") || "-"}</td>
              <td className="text-right">
                <form action={toggleActive} className="inline">
                  <input type="hidden" name="id" value={c.id} />
                  <button className="underline mr-3">{c.active ? "Deactivate" : "Activate"}</button>
                </form>
                <form action={deleteCoupon} className="inline" onSubmit={(e) => { if(!confirm('Delete coupon?')) e.preventDefault(); }}>
                  <input type="hidden" name="id" value={c.id} />
